...
pubeval(int race, int pos[])
{
  ...
  for(j = 24, i = 0;  j > 0; --j, i += 5) {
    ...
    switch( n ) {
      case -1: score += wc[i]; break;
      case  1: score += wc[i+1]; break;
      case  2: score += wc[i+2]; break;
      case  3: score += wc[i+2] + wc[i+3]; break;
#if defined(__GNUC__)
      case  4 ... 15:
#else
