 ...
 * MoneyEval authors: Aaron Tikuisis and Isaac Keslassy, 2020
 *
 ...
 *
 * $Id: gtkcube.c,v 1.97.4.1 2020/11/25 05:02:18 pif Exp $
 */
...
#include <stdlib.h>
#include <string.h>
#include <glib.h> //missing, even though mentioned in several .c files
...
- One minor problem is left: When running a rollout in MoneyEval, the "JSDs" column displays "nan"
 - TBD Aaron: Explain & ask about ChangeGame(NULL)?
*/
...
// The second variable determines whether to use a Jacoby assumption or not
...
TakeAnalysis(cubehintdata * pchd)
{
    ...
    if (pchd->ms.nMatchTo && pchd->evalAtMoney) {
        ...
        gtk_widget_set_tooltip_text(pwJ, _("Toggle Jacoby option for money play"));
...
*/
    ...
    //g_new0(struct_type, n_structs) allocates n_structs elements of
    // type struct_type , initialized to 0's. The returned pointer is
    // cast to a pointer to the given type.
