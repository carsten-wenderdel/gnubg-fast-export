 ...
 *
 * $Id: gtkscoremap.c,v 1.2 2021/10/05 20:58:46 plm Exp $
 */
...
typedef enum _colourbasedonoptions { ALL, DND, PT} colourbasedonoptions;
...
typedef enum _displayevaloptions { NO_EVAL, CUBE_ND_EVAL, CUBE_DT_EVAL, CUBE_RELATIVE_EVAL } displayevaloptions;
...
/* Used in the "Display Eval" radio buttons - we assume the same order as the labels */
typedef enum _layoutoptions { VERTICAL, HORIZONTAL } layoutoptions; 
...
DrawGauge(GtkWidget * pw, int i, colourbasedonoptions toggledColor) {
    ...
    if (toggledColor == ALL){
        ...
        dt = Interpolate(0.5f*(float)i/(float)GAUGE_SIXTH_SIZE, 0.0f, 1.0f, 1.0f, 1.0f-maxDiff, 2.0f, 1.0f+maxDiff, 3.0f, 1.0001f); //epsilon added at the end to distinguish TGTD from ND
        ...
        if (psm->cubeScoreMap) {
            ...
            if (labelBasedOn == LABEL_AWAY) { 
                ...
                if (i2<psm->tableSize-2) {
                    sprintf(sz, _("%d"), i2);
                    sprintf(longsz, _(" %d/%d "), i2,MATCH_SIZE(psm));
    ...
    if (pq->isAllowedScore==ALLOWED) { // non-greyed quadrant
        ...
        if (pq->isTrueScore) { //emphasize the true-score square 
            ...
            if (pq->ci.nMatchTo==0)
                ...
                strcat(buf,"<span size=\"x-small\">(Current score)</span>\n"); // or "(Same score)"
...
//  GtkWidget *pwhbottom;
    ...
    if (psm->cubeScoreMap) {    //========== cube scoremap
        ...
        pw = pwx = gtk_radio_button_new_with_label(NULL, _("1"));
        gtk_box_pack_start(GTK_BOX(psm->pwCubeBox), pw, FALSE, FALSE, 0);
        pi = (int *)g_malloc(sizeof(int)); 
        *pi=1;
        g_object_set_data_full(G_OBJECT(pw), "user_data", pi, g_free);
